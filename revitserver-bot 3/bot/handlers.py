...
async def cb_prices(callback):
    ...
    await callback.message.answer(
        "💰 *Тарифы Revit Server:*\n\n"
        "• *LOD 100* — 2–4 чел. → 8 900 ₽/мес\n"
        "• *LOD 200* — 5–8 чел. → 11 200 ₽/мес\n"
        "• *LOD 300* — 9–15 чел. → 14 600 ₽/мес\n"
        "• *LOD 400* — от 15 чел. → от 17 500 ₽/мес\n\n"
        "💻 *Виртуальное рабочее место (VDI):*\n\n"
        "• *BIM-Start* → 10 500 ₽/мес\n"
        "• *BIM-Standart* → 12 500 ₽/мес\n"
        "• *BIM-Pro* → 16 000 ₽/мес\n\n"
        "Все тарифы включают бэкап и сетевой диск. "
        "Напишите сколько человек в команде — подберу оптимальный вариант 👇",
        parse_mode="Markdown",
    )
...
async def cb_trial(callback):
    ...
    await callback.message.answer(
        ...
        "Без риска, без предоплаты. За это время вы:\n"
        "✅ Подключите свою команду\n"
        "✅ Откроете реальный проект\n"
        "✅ Убедитесь в скорости и стабильности\n\n"
        "Чтобы начать — напишите сколько человек в вашей команде "
        "и какую версию Revit используете, оформим доступ.",
        parse_mode="Markdown",
    )
...
async def cb_revit_server(callback):
    await callback.answer()
    await callback.message.answer(
        "🖥 *Revit Server* — совместная работа над BIM-проектами\n\n"
        "Revit Server — серверный компонент Autodesk, который позволяет "
        "нескольким пользователям работать с одной моделью одновременно.\n\n"
        "Мы размещаем и администрируем сервер в облаке:\n"
        "• Готово за 1–2 часа после оплаты\n"
        "• Независимость от Autodesk (не заблокируют как BIM360)\n"
        "• Ежедневные бэкапы + сетевой диск в подарок\n"
        "• Помогаем настроить RSN и VPN на каждой машине\n\n"
        "Сколько BIM-специалистов в вашей команде?",
        parse_mode="Markdown",
    )
...
async def cb_vdi(callback):
    ...
    await callback.message.answer(
        "💻 *Виртуальное рабочее место (VDI)*\n\n"
        "Полноценный Windows-компьютер в облаке с предустановленным Revit, "
        "AutoCAD и другими САПР.\n\n"
        "Подходит если:\n"
        "• Слабое железо на местах\n"
        "• Удалённая команда / разные офисы\n"
        "• Нужен доступ с любого устройства\n\n"
        "Тарифы от 10 500 ₽/мес за рабочее место.\n\n"
        "Расскажите подробнее о вашей задаче — подберём конфигурацию.",
        parse_mode="Markdown",
    )
...
async def _escalate(
    ...
    if notify_user:
        await message.answer(
            ...
            "Пока ожидаете, можете также написать напрямую: @revitserver"
        )
